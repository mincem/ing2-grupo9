...
\QA
  {Seguridad (Confidencialidad)} % tipo de atributo
  {La información almacenada dentro de los SmartTV debe ser segura} % texto del atributo
  {Atacante externo} % fuente
  {Logra acceder a los datos confidenciales que almacena un SmartTV} % estímulo
  {Datos de SmartTV} % artefacto
  {Operación normal} % entorno
  {El atacante no puede decodificar los datos confidenciales} % respuesta
  {En el 99,9\% de los casos el atacante necesitaría más de 10 años para decodificar los datos} % medida de la respuesta
